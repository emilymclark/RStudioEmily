...
ob_sent$topat <- ifelse(str_detect(ob_sent$sentences,"patient|treat|pain|suffer|relief|relieve")==TRUE,"patient",0)
ob_sent$tosoc <- ifelse(str_detect(ob_sent$sentences,"societ|communit|public|law")==TRUE,"tosoc",0)
  #Counts number of patient obligation sentences
topat_count <- ob_sent$topat %>% count("patient")
